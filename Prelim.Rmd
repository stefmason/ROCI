...
#Cox regressions
cox_qt <- coxph(Surv(survival, censor) ~ strata(quartile), data=imgs) #for graph
cox_quart <- coxph(Surv(survival, censor) ~ Age+Male+Immsupp+non_white+APACHE+quartile, data=imgs) #for betas
summary(cox_quart)

schoenfeld_res <- cox.zph(cox_quart)
plot(schoenfeld_res)
```
...
cox_all_non <- coxph(Surv(survival, censor) ~ strata(quartile), data=imgs_non)
summary(cox_all_non)
```
